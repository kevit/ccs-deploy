...
use vars qw(
   ...
   $scheme  $authority  $path  $host  $port  $username  $password  $csp  $csp_ro
...
if ( $is_html  && ($body ne '') && !$response_sent ) {
    ...
    # This is for a non-English insertion.
    $charset= 'utf-8'  if $lang ne 'en' and $lang ne '' ;
...
#		while ($attrs=~ /([A-Za-z][\w.:-]*)\s*(?:=\s*(?:"([^">]*)"?|'([^'>]*)'?|([^'"][^\s>]*)))?/g ) {
		...
		$attr{src}=      &full_url_by_frame($attr{src}, 1, 1), $rebuild=1 if defined($attr{src}) ;
		$attr{longdesc}= &full_url($attr{longdesc}),           $rebuild=1 if defined($attr{longdesc}) ;
	...
	#   a Perl-to-JavaScript translation of this block.
	    ...
	    next if $script_block=~ /\S/ and !match_csp_source_list('script-src', "'unsafe-inline'", $attr{nonce}) ;
...
sub proxify_expressions_in_css {
	...
	# match_csp_source_list() may die, so only call when needed.
...
#	    } until (/^(\015\012|\012)$/) || eof(S) ; # lines end w/ LF or CRLF
	...
	# For text files, default to charset=UTF-8 so that inserted form displays correctly.
	if (!$charset and $content_type=~ m#^text/#) {
	    $charset= 'UTF-8' ;
	    $headers=~ s/^(Content-Type:[^\015\012]*)/$1; charset=UTF-8/gmi ;
	}
...
sub http_fix {
    ...
    # Add our CSP headers, for both standard and Firefox.
    # These wouldn't be safe, except that we enforce the directives elsewhere.
    # jsm-- these aren't fully supported by browsers yet; add back in when they are.
    # jsm-- should allow data: in various directives where incoming CSP allows it?
#    my $csp_out= "default-src $THIS_SCRIPT_URL/ 'unsafe-inline' 'unsafe-eval' ; img-src $THIS_SCRIPT_URL/ data: ; form-action $THIS_SCRIPT_URL/ ; base-uri $THIS_SCRIPT_URL/" ;
#    unshift(@headers, "Content-Security-Policy: $csp_out\015\012") ;
#    unshift(@headers, "X-Content-Security-Policy: $csp_out\015\012") ;
#    unshift(@headers, "X-Webkit-CSP: $csp_out\015\012") ;
...
sub new_header_value {
    ...
    # jsm jsm jsm-- MUST ENFORCE THIS ELSEWHERE!!! OR ELSE SCRIPT IS INSECURE!!!
    ...
    #   reflected-xss
    #   report-uri
    if ($name eq 'content-security-policy' or $name eq 'x-webkit-csp' or $name eq 'x-content-security-policy') {
return undef ;  # we don't support this while browser support is incomplete
	# Reject this in <meta> tag if $csp already exists-- see CSP spec, section 3.1.3 .
	return undef if $is_meta_tag and $csp ;
	$value= parse_csp_header($value) ;
	return $value ;
    }
...
# This is done according to the CSP 1.1 spec as of 3-16-2013 .
...
sub parse_csp_header {
    ...
    # Build new header from non-source-list directives.
    ...
    my $new_header= join('; ', map {"$_ @{$new_policy->{$_}}"}
				   grep {$new_policy->{$_}}
					qw(sandbox script-nonce plugin-types reflected-xss) ) ;
...
sub match_csp_source_list {
    ...
    my($match) ;
return 1 ;  # we don't support this while browser support is incomplete
	...
	foreach my $directive (@{$csp->{$directive_name}}) {
	    ...
	    &HTMLdie("CSP $directive_name violation at line " . (caller)[2] . " for $uri: [@$directive]", 'CSP violation'), return 0
		unless $match ;
	}
	return 1 ;
    }
...
var _proxy_jslib_cookies_are_banned_here, _proxy_jslib_doing_insert_here, _proxy_jslib_SESSION_COOKIES_ONLY,
    ...
    _proxy_jslib_csp, _proxy_jslib_csp_st, _proxy_jslib_eval_ok ;
...
function _proxy_jslib_pass_vars(base_url, origin, cookies_are_banned_here, doing_insert_here, SESSION_COOKIES_ONLY, COOKIE_PATH_FOLLOWS_SPEC, RESPECT_THREE_DOT_RULE, ALLOW_UNPROXIFIED_SCRIPTS, RTMP_SERVER_PORT, default_script_type, default_style_type, USE_DB_FOR_COOKIES, PROXIFY_COMMENTS, COOKIES_FROM_DB, csp) {
...
//alert('ending Document.close()') ;
	...
	case 'value':
		...
		return _proxy_jslib_proxify_attribute(o, o.name, o.value, 1) ;
	    } else {
		return _handle_default() ;
	    }
...
// A horrible, horrible hack to make busy sites not crash.
time*= 10 ;
...
//if (o==null) alert('in assign, o is null, property, caller=\n['+property+']\n['+arguments.callee.caller+']') ;   // jsm-- remove in production release?
	...
	case 'src':
		...
		} else if (o_tagname=='frame' || o_tagname=='i_frame') {
...
function _proxy_jslib_proxify_attribute(element, name, value, reverse) {
	...
	return _proxy_jslib_full_url_by_frame(value, null, true, reverse,
					      name.toLowerCase()=='src' && element.nodeName.match(/^i?frame$/i) ) ;
...
return true ;  // we don't support this while browser support is incomplete
...
sub install_modules {
    ...
    @modules= qw(Net::SSLeay  JSON  IO::Compress::Gzip  IO::Compress::Deflate)  unless @modules ;
